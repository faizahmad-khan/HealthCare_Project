...
from .forms import DispensingForm, MedicineCategoryForm, MedicineForm, StockMovementForm
from .models import Dispensing, Medicine, MedicineCategory
...
def medicine_list(request):
    ...
    if low_stock:
        medicines = [medicine for medicine in medicines if medicine.is_low_stock]
...
def medicine_update(request, pk):
    ...
    return render(
        ...
        {'form': form, 'title': 'Update Medicine', 'medicine': medicine},
    )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if dispensing.quantity > medicine.stock_quantity:
                ...
                messages.success(
                    request,
                    f'Medicine dispensed successfully! Total: {dispensing.total_price}',
                )
...
def pharmacy_dashboard(request):
    ...
    low_stock_medicines = [medicine for medicine in active_medicines if medicine.is_low_stock]
    expired_medicines = [medicine for medicine in active_medicines if medicine.is_expired]
    recent_dispensings = Dispensing.objects.select_related('medicine').order_by('-dispensed_at')[:10]
    total_dispensed_today = (
        Dispensing.objects.filter(dispensed_at__date=date.today()).aggregate(total=Sum('total_price'))['total']
        or 0
    )
